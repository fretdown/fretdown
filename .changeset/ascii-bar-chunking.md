...
region, so trailing annotations like `(6x)` are no longer misread as notes — and a trailing
repeat count (`(6x)`, `x6`) is captured as a repeat (`|: … :|x6`) instead of being dropped.
