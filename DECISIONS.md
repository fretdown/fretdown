...
- **Playback uses `webaudio-tinysynth` (a self-contained GM synth), not a sample/soundfont loader** — no external sample files to fetch, works offline, and exposes all 128 GM instruments by program number so the per-track instrument dropdown is a single MIDI program change. `webaudio-tinysynth` is dynamically `import()`-ed inside the player so it stays out of the `/play` first-load bundle.
