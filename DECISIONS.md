...
- **A connector chain (e.g. `s5f2h3`) expands into multiple `TabNote`s joined by VexFlow connectors** — hammer/pull use `TabTie.createHammeron`/`createPulloff`, slides use `TabSlide.createSlideUp`/`createSlideDown`, drawn after the voice is laid out (they read note coordinates). The beat's duration is subdivided evenly across the chain so ticks stay aligned, but only when the chain length is a power of two and the per-note value is ≤ a 32nd; dead notes, non-transition connectors (`b`/`r`), and chains inside tuplets fall back to the single-note + annotation rendering. Dotted notes expand too (splitting a dotted value keeps the dot), and odd chain lengths (e.g. 3) expand as a tuplet — `count` notes in the `powerOfTwoBelow(count)` space. Bends/releases (`b`/`r`) don't create noteheads — they decorate the current notehead as `Bend` arrows — so a mixed chain like `s3f9b11r9p7` draws bends on fret 9 and a pull-off tie to fret 7, with no leftover text label; only transition connectors add noteheads. Chords with a shared transition chain (every note has the same transition connectors, e.g. `(s5f17\16 s6f15\14)`) expand into stacked chord noteheads with a per-string slide/tie (`Connection.index` selects the position). The tuplet's tick ratio is applied for spacing; like all tab tuplets here, no visible bracket is drawn (TabNotes have no stems). Bends still attach a `Bend` modifier to one note (a bend is a single attack). _(Supersedes the original v1 limitation of rendering only the first fret + a text annotation.)_
...
- **Vibrato renders as VexFlow's `Vibrato` wavy line; palm mute as a lifted "P.M." label** — VexFlow has a vibrato primitive but no palm-mute glyph, so `.vib` → a `Vibrato` modifier (the standard squiggle) and `.pm` → a "P.M." annotation shifted up clear of the top string line (other articulations stay text labels). Keeps technique marks from crowding the first fret.
...
- **Repeated bends collapse to one arrow** — `addBendModifiers` drops consecutive bends to the same fret (so `b11 b11` is a single "Full" arrow, not "Full Full") before folding a trailing release into the bend; shared by the chain and single-note paths.
...
- **Vercel Web Analytics via `@vercel/analytics`** — the `<Analytics />` component is mounted once in the root layout; it self-activates on Vercel deployments (data collection must be toggled on once in the project's Analytics tab).
...
- **Playback uses a small sample-based engine over raw Web Audio (free FluidR3_GM samples via jsDelivr), replacing `webaudio-tinysynth`** — samples give realistic guitar/bass timbre, and **per-note bends/vibrato are done by automating each voice's `AudioBufferSourceNode.detune`** (which is why neither `Tone.Sampler` nor `smplr` worked — neither exposes per-voice pitch). The instrument id's `sample` field (core) picks the FluidR3 folder; samples load per playing channel (~one per minor third, pitch-shifted to fill gaps). If a channel's samples fail to load it falls back to a sawtooth oscillator voice (still bendable), so playback is never silent. Bends now live **per-event** on the timeline (not a channel stream) so detune is per-voice; Stop mutes the master gain to kill scheduled voices. _(Supersedes the tinysynth + channel-pitch-bend notes below.)_
...
- **Muted/dead notes play a short, soft thunk, not silence** — a dead note emits a brief (≤80ms) low-velocity note on the open string so the percussive mute is audible in playback, instead of being skipped.
- **Bends/releases glide via MIDI pitch-bend; only transitions re-attack** — `b`/`r` decorate a single sustained attack with a pitch-bend ramp (matching how the renderer keeps them on one notehead), while hammer/pull/slide stay separate note-ons. The synth's bend range is widened to ±12 semitones via RPN; the timeline carries a separate sorted `bends` stream that the player schedules and re-centers on stop. A tempo override (playground slider) recomputes `barSeconds`.
...
- **Trailing connectors/`~` with no target become events or articulations** — `12\`/`3b`/`3/` (a connector at the end of a note with no following fret) map to a default-target slide/bend, `~`/`~~~` to a `.vib` articulation (its length emerges from the dash spacing → note duration). Previously these were silently dropped.
- **Palm mute is read from a dots line above a system** — a non-tab line of `.`s (e.g. `  .  .  .  .`) marks those columns `.pm` on the notes below (raw columns mapped into bar-relative body coords, ±1 tolerance).
- **Each ASCII system is laid out and repeated on its own** — systems become their own bars (rest-padded) and keep their own `xN` repeat, so stacked riffs with different counts (`4x`, `8x`) don't collapse into one. Palm-mute/vibrato are also played back (muted = short + soft; vibrato = a gentle pitch wobble).
- **The importer ignores ASCII barlines and chunks notes into bar-sized measures, padding the last with rests** — ASCII barline spacing rarely matches real measures, so trusting them produced measures that didn't fill the time signature (a validation error). Chunking by `eighthsPerBar` (8 for 4/4) always yields valid measures. It also trims each line to the first–last `|` so trailing annotations like `(6x)` aren't misread as notes; a trailing repeat count (`(6x)`/`x6`) is instead captured as a repeat (`repeatStart`/`repeatEnd.times`) wrapping the riff.
- **Multi-system tabs are concatenated, not reduced to the longest block** — real tabs stack many systems (line groups) separated by blank lines; the importer collects every group whose size matches the dominant string count and plays them back to back (`multi-system` flag), rather than keeping only one. Per-system rhythm is inferred independently, then the combined onset stream is laid onto bars.
- **Note durations are inferred from horizontal spacing, not fixed to eighths** — the tightest gap between two notes is treated as one eighth and a note is held until the next note's column, so wider spacing yields longer notes (quantized to standard values, split greedily at bar lines). It's still approximate, but captures the riff's feel rather than a flat eighth-note stream.
- **Repeats are expanded for display/playback via `expandRepeats`, not by drawing repeat signs** — the renderer and playback consume `expandRepeats(score)`, so a `|: … :|x6` riff is shown and heard as six literal bars (what a casual user expects), while the editor source stays compact. Voltas aren't handled yet.
